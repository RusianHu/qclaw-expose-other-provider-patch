...
- 对 `QClaw 0.1.19 / 0.1.20` 自动执行 `skillhub-installer.ts` 的 regex 兼容热修补
...
- 已新增 `0.1.19 / 0.1.20` 的 `other guard` 特征兼容识别
- 已内嵌 `0.1.19 / 0.1.20` 的 `skillhub_install` regex 兼容修复
- 其中 `0.1.20` 已兼容 runtime 仍旧旧式、schema 已收敛为 `^[\\w\\-\\.]{1,128}$` 的过渡状态
...
- `QClaw 0.1.19 / 0.1.20` 已启用 Electron ASAR 完整性校验；仅修改 `resources/app.asar` 而不同步完整性元数据，会导致程序无法启动
- 当前脚本会同步更新 ASAR 头部中的目标文件完整性记录，并回写 `QClaw.exe` 内嵌的 `ELECTRONASAR` 头部哈希
- 对 `skillhub_install` 的 regex 修复不是外置二次脚本，而是主脚本在同一执行流内按版本/特征自动判定后顺带执行；`0.1.20` 的过渡 schema 也已纳入兼容识别
