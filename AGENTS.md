...
- 已重新适配 `QClaw 0.2.4`；默认要求版本匹配，也支持显式放宽版本限制后再做特征校验。
- 保留对旧 `0.1.16 / 0.1.13` guard 特征的兼容识别，并内嵌 `skillhub_install` regex 兼容修复。
- `QClaw 0.2.4` 除静态 `doubao` 槽位外，还会通过 `modelApi` 远端返回值覆盖本地下拉；主脚本现已内嵌该覆盖禁用修补。
...
- 逆向确认 `QClaw 0.2.4` 在本地静态 provider 列表之外，还会通过 `modelApi` 远端返回值覆盖 UI 下拉；只做 `doubao -> other` 不足以稳定生效。
- `0.1.19 / 0.1.20 / 0.1.22 / 0.2.1 / 0.2.4` 中 `other` guard 压缩变量名均已纳入兼容特征集合。
- 当前采用**双位点等长原位补丁**，避免完整解包/重打 `app.asar`：
  - provider 槽位：`key:"doubao",label:"火山引擎（豆包）"` → `key:"other",label:"其他"`
  - 远端覆盖位点：`t.data&&t.data.length>0&&(Eo=t.data,hu(Eo,"modelApi"))` → `t.data&&t.data.length<0&&(Eo=t.data,hu(Eo,"modelApi"))`
- 已定位并修复 `QClaw 0.1.19 / 0.1.20 / 0.1.22 / 0.2.1 / 0.2.4` 内置 `skillhub_install` 工具的 regex 兼容性问题；其中 `0.1.20 / 0.1.22 / 0.2.1 / 0.2.4` 兼容 runtime 旧式 + schema 过渡态，该修复现已内嵌到主脚本，按版本/特征自动执行。
...
5. 若命中 `0.1.19 / 0.1.20 / 0.1.22 / 0.2.1 / 0.2.4` 且检测到兼容的 `skillhub_install` regex 旧特征或过渡特征，则在**同一执行流**内顺带修补 `skillhub-installer.ts`。
6. `QClaw 0.2.4` 若仅完成 provider 槽位替换、但 `modelApi` 远端覆盖仍未禁用，则应视为**半修补状态**并继续补齐。
...
- 软件更新后大概率需要重新评估；本次已确认 `0.2.4` 的主要变化包括新的 `other` guard 压缩片段、`skillhub_install` 延续 runtime 旧式 + schema 过渡态，以及 `modelApi` 动态 provider 覆盖。
- 该补丁本质上仍是**替换一个现有固定厂商槽位**为“其他”，不是在列表末尾真正新增新项。
- `QClaw 0.2.4` 若只改动 provider 槽位而未禁用 `modelApi` 远端覆盖，UI 仍可能显示“火山引擎（豆包）”；当前主脚本已将该二阶段修补并入同一执行流。
