...
- 已重新适配 `QClaw 0.1.20`；默认要求版本匹配，也支持显式放宽版本限制后再做特征校验。
...
- `0.1.19 / 0.1.20` 中 `other` guard 压缩变量名均已纳入兼容特征集合。
- 采用**等长原位补丁**，避免完整解包/重打 `app.asar`：
  - 原始特征：`key:"doubao",label:"豆包"`
  - 替换目标：`key:"other",label: "其他"`
- 已定位并修复 `QClaw 0.1.19 / 0.1.20` 内置 `skillhub_install` 工具的 regex 兼容性问题；其中 `0.1.20` 兼容 runtime 旧式 + schema 过渡态，该修复现已内嵌到主脚本，按版本/特征自动执行。
...
5. 若命中 `0.1.19 / 0.1.20` 且检测到兼容的 `skillhub_install` regex 旧特征或过渡特征，则在**同一执行流**内顺带修补 `skillhub-installer.ts`。
...
- 软件更新后大概率需要重新评估；本次已确认 `0.1.20` 的主要变化包括新的 `other` guard 压缩片段，以及 `skillhub_install` 出现 runtime 旧式 + schema 过渡态。
